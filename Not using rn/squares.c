...
void sort_squares(int *arr, int n){
    ...
    for(int i = n - 1; i >= 0; i--){
        if(absolute(arr[left]) > absolute(arr[right])){
            //fill code here
            result[i] = arr[left] * arr[left]; //store square in the result
            left += 1; //go next index to right
        }
        else{
            //fill code here
            result[i] = arr[right] * arr[right]; //store square in the result
            right -= 1; //go next index to left
        }
    }
