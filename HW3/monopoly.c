...
{
	if((*p1).balance - amount >= 0) //p1 has enough money
	{
		(*p1).balance -= amount; //p1 pays
        (*p2).balance += amount; //p2 recieves
        return 1; //true they had enough money
	}
	else
	{
		(*p2).balance += (*p1).balance; //p2 gets p1 assets
        (*p1).balance = 0; //p1 is now broke
        return 0; //false they had enough money
	}
}
...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		int steps = rand() % 6 + 1 + rand() % 6 + 1; //rolling 2 dice
		//fill in the code below

		if((p[i].loc + steps) >= n) //player passes go
		{
			int steps_left = steps - (n - p[i].loc); //steps needed after passing go
			p[i].loc = 0; //player is now at 0
			p[i].balance += n; //collect your reward
			p[i].loc += steps_left; //keep going past Go
		}
		else
		{
			p[i].loc += steps; //player takes steps but did not pass Go
		}


		TProperty *landed_property = &prop[p[i].loc]; //see if we landed on owned property
		
		if (landed_property->owner_id == -1) //property is unownded
		{
            landed_property->owner_id = i; //players buy the property
        }
		else //property is owned
		{
            if (transaction(&p[i], &p[landed_property->owner_id], landed_property->rent) == 0) //player cannot pay
			{
                return 0; //game ends
            }
        }
	...
	return 1; //player completed the round
}
