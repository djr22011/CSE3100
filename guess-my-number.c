...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int max = gmn_get_max();
    if(write(fdc[PIPEFD_WRITE], &max, sizeof(int)) != sizeof(int)) //needs to write enough bytes to pass to pipe
    {
        die("Could not send max value to parent"); //process only dies if things fail to go through
    } 
    ...
    do
    {
        if (read(fdp[PIPEFD_READ], &guess, sizeof(int)) != sizeof(int)) //write enough bytes to see the guess
        {
            die("Could not get guess from parent");
        }

        result = gmn_check(&gmn, guess); //check to see if guess was right, high, or low

        if (write(fdc[PIPEFD_WRITE], &result, sizeof(int)) != sizeof(int)) //write enough bytes to see result
        {
            die("Could not send result to parent");
        }
        
    } while(result != 0); //result of 0 means guess was correct
    
    //  send the final message back (as a string) 
    if (write(fdc[PIPEFD_WRITE], gmn_get_message(&gmn), strlen(gmn_get_message(&gmn))) == -1) //write num attempts and correct number
    {
        die("Could not send final message to parent");
    }
...
int main(int argc, char *argv[])
{
    ...
    while (read(fdc[PIPEFD_READ], &c, 1) == 1 && c != '\n' && bytes < MSG_BUF_SIZE - 1) //gives each character 1 byte and assigns it to string 
                                                                                        //with the final message in it
    {
        final_message[bytes++] = c; //update number of bytes the string contains
    }

    final_message[bytes] = '\n'; //newline at end of message
    final_message[bytes + 1] = '\0'; //put null terminator at end of message
    fputs(final_message, stdout); //fputs is like printf
